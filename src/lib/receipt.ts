...
function receiptHeaderHtml(storeName: string, whatsapp?: string | null, cnpj?: string | null) {
  return [
...
// Tamanho de bobina térmica com altura automática — sem isso o navegador usa
// o tamanho de folha padrão (A4/Carta) e desperdiça bobina imprimindo uma
// página inteira pra um cupom pequeno. Texto em negrito e maior porque fontes
// finas saem fracas na cabeça de impressão térmica. Compartilhado entre o
// cupom de venda e o comprovante de pagamento do crediário — mesmo papel,
// mesma impressora.
function receiptDocument(paperMm: number, bodyHtml: string) {
  return `
...
export function buildReceiptHtml(params: {
  ...
  return receiptDocument(
    paperMm,
    `
    ${receiptHeaderHtml(storeName, whatsapp, cnpj)}
...
export function buildCreditPaymentReceiptHtml(params: {
  storeName: string;
  whatsapp?: string | null;
  cnpj?: string | null;
  paperMm: number;
  ...
  return receiptDocument(
    ...
    ${receiptHeaderHtml(storeName, whatsapp, cnpj)}
    <p class="muted">${new Date().toLocaleString("pt-BR")}</p>
    <p class="muted">Comprovante de pagamento — Crediário</p>
    <div class="divider"></div>
    ...
    <div class="divider"></div>
    <p class="row"><span>Forma de pagamento</span><span>${escapeHtml(paymentMethodLabel)}</span></p>
    <p class="row"><span>Saldo restante</span><span>${formatCurrency(balanceAfter)}</span></p>
    <p class="footer">Obrigado pela preferência!</p>
    `,
  );
}
