...
export default function Kits() {
  ...
  async function uploadKitPhoto(file: File): Promise<string | null> {
    ...
    const path = `${store.id}/kit-${crypto.randomUUID()}.jpg`;
    const { error: uploadError } = await getSupabase().storage.from("product-images").upload(path, blob, { contentType: "image/jpeg" });
    if (uploadError) {
  ...
  async function handleNewKitPhoto(e: React.ChangeEvent<HTMLInputElement>) {
...
function KitPhotoButtons({
  ...
  return (
    <>
      ...
      {kit.image_url && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={kit.image_url} alt="" className="h-10 w-10 rounded-lg object-cover" />
      )}
    </>
  );
}
