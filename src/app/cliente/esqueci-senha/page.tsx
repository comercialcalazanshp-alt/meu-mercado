...
import { requestCustomerPasswordReset } from "@/app/cliente/actions";

function EsqueciSenhaClienteForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const result = await requestCustomerPasswordReset(
      email,
      `${window.location.origin}/cliente/redefinir-senha${loja ? `?loja=${loja}` : ""}`,
    );
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 bg-slate-50 px-6 py-24 text-center dark:bg-slate-950">
        ...
        <p className="max-w-sm text-sm text-slate-600 dark:text-slate-400">
          Se {email} tiver uma conta cadastrada, chega um link pra redefinir a senha em instantes. Isso
          também libera a conta caso ela tivesse sido bloqueada por tentativas erradas.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-1 flex-col items-center justify-center bg-slate-50 px-6 py-16 dark:bg-slate-950">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-slate-900 dark:text-slate-50">Esqueceu sua senha?</h1>
...
export default function EsqueciSenhaCliente() {
  return (
    <Suspense fallback={null}>
      <EsqueciSenhaClienteForm />
    </Suspense>
  );
}
