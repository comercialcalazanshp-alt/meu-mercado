-- Meu Mercado — v99.
-- Pedido mínimo pra entrega (não afeta retirada na loja). Vem ligado por
-- padrão com R$25, mas cada loja (Hub, afiliado ou loja única — é campo
-- em "stores", não em "affiliate_partnerships", já que cada uma tem seu
-- próprio painel/bairros pra ajustar) pode religar/desligar e mudar o
-- valor à vontade.
alter table public.stores add column if not exists min_order_for_delivery_enabled boolean not null default true;
alter table public.stores add column if not exists min_order_for_delivery numeric not null default 25;
