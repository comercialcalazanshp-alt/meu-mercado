-- Meu Mercado — v102.
-- Frete grátis a partir de um valor de compra (liga/desliga + valor
-- ajustável por loja, mesmo padrão do pedido mínimo — v99). Quando o
-- subtotal bate o valor configurado, o frete daquele bairro vira R$0,
-- sem mexer no valor cadastrado em "Frete por bairro" (só zera na hora
-- do checkout).
alter table public.stores add column if not exists free_delivery_threshold_enabled boolean not null default false;
alter table public.stores add column if not exists free_delivery_threshold numeric not null default 0;
