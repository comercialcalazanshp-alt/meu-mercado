-- Meu Mercado — v118.
-- CORREÇÃO DE SEGURANÇA/DINHEIRO: checkout() creditava cashback e bônus de
-- indicação na hora da compra, mesmo em Pix/cartão online — que só são
-- confirmados de verdade depois, via webhook. Um cliente podia fazer um
-- pedido em Pix, nunca pagar, e o cashback já caía na conta dele mesmo
-- assim — dinheiro de graça, repetível. A comissão do afiliado já tinha
-- sido corrigida pra esperar confirmação de pagamento (ver v92/v97); essa
-- migração aplica a mesma regra pro cashback e pro bônus de indicação.
--
-- "Dinheiro" e "Cartão na entrega" continuam creditando na hora (são
-- pagos ali, sem confirmação assíncrona). Pix e cartão online passam a
-- guardar o valor no pedido e só creditar quando o pagamento for
-- confirmado de verdade (mesmo ponto onde a comissão do afiliado hub já é
-- lançada hoje).

alter table public.orders add column if not exists referral_bonus_earned numeric not null default 0;
alter table public.orders add column if not exists referrer_customer_id uuid references public.customers(id);
alter table public.orders add column if not exists cashback_credited_at timestamptz;
...
begin
  ...
  v_payment_method := coalesce(nullif(trim(p_payment_method), ''), 'dinheiro');
  -- Só métodos pagos na hora (sem confirmação assíncrona) creditam
  -- cashback/bônus de indicação imediatamente. Pix e cartão online ficam
  -- pendentes até o webhook de pagamento confirmar (credit_pending_cashback).
  v_is_instant := v_payment_method in ('dinheiro', 'cartao_entrega');

  if exists (select 1 from public.customer_notes where store_id = p_store_id and phone = v_phone and blocked = true) then
    raise exception 'Não foi possível concluir esse pedido. Fale com a loja.';
  end if;

  select cashback_percent, referral_bonus, min_order_for_delivery_enabled, min_order_for_delivery,
         free_delivery_threshold_enabled, free_delivery_threshold
  ...
  if p_neighborhood_id is not null and v_store.min_order_for_delivery_enabled and v_subtotal < v_store.min_order_for_delivery then
    raise exception 'Pedido mínimo pra entrega é de R$ % (faltam R$ %)', v_store.min_order_for_delivery, round(v_store.min_order_for_delivery - v_subtotal, 2);
  end if;

  if p_neighborhood_id is not null and v_store.free_delivery_threshold_enabled and v_store.free_delivery_threshold > 0 and v_subtotal >= v_store.free_delivery_threshold then
    v_delivery_fee := 0;
  end if;
  ...
  insert into public.orders (
    ...
    referral_bonus_earned, referrer_customer_id, cashback_credited_at
  )
  values (
    p_store_id,
    trim(p_customer_name),
    ...
    v_referrer_id,
    case when v_is_instant then now() else null end
  )
...
-- Credita o cashback/bônus de indicação que ficou pendente num pedido
-- pago com Pix/cartão online, chamada pelo servidor (nunca pelo cliente)
-- no exato momento em que o webhook de pagamento confirma que caiu de
-- verdade. Idempotente: se já creditado (cashback_credited_at preenchido),
-- não faz nada — seguro pra chamar de novo numa retentativa de webhook.
create or replace function public.credit_pending_cashback(p_order_id uuid)
returns void
language plpgsql
security definer
set search_path to 'public'
...
declare
  v_order record;
begin
  select customer_id, cashback_earned, referral_bonus_earned, referrer_customer_id, cashback_credited_at
    into v_order
    from public.orders
    where id = p_order_id
    for update;

  if not found or v_order.cashback_credited_at is not null or v_order.customer_id is null then
    return;
  end if;

  if v_order.cashback_earned > 0 then
    update public.customers set cashback_balance = cashback_balance + v_order.cashback_earned
      where id = v_order.customer_id;
  end if;

  if v_order.referral_bonus_earned > 0 and v_order.referrer_customer_id is not null then
    update public.customers set cashback_balance = cashback_balance + v_order.referral_bonus_earned
      where id in (v_order.customer_id, v_order.referrer_customer_id);
  end if;

  update public.orders set cashback_credited_at = now() where id = p_order_id;
end;
$$;

-- Essa função não valida quem está chamando (confia que só o servidor,
-- depois de confirmar o pagamento de verdade via webhook, vai chamá-la) —
-- por isso, ao contrário de checkout()/checkout_hub(), NUNCA pode ser
-- liberada pra anon/authenticated. Revoga explicitamente, independente do
-- privilégio padrão do banco.
revoke execute on function public.credit_pending_cashback(uuid) from public, anon, authenticated;

alter table public.orders add column if not exists cashback_reversed_at timestamptz;

-- Desfaz o cashback/bônus de indicação de um pedido (usado tanto por
-- cancelamento do cliente quanto por estorno manual do dono) — devolve o
-- que foi gasto e desconta o que já tinha sido creditado, sem deixar a
-- conta negativa. Idempotente via cashback_reversed_at: chamar de novo
-- pro mesmo pedido não desconta duas vezes.
create or replace function public.reverse_order_cashback(p_order_id uuid)
returns void
language plpgsql
security definer
set search_path to 'public'
...
declare
  v_order record;
begin
  select customer_id, cashback_earned, cashback_used, referral_bonus_earned, referrer_customer_id,
         cashback_credited_at, cashback_reversed_at
    into v_order
    from public.orders where id = p_order_id;

  if not found or v_order.customer_id is null or v_order.cashback_reversed_at is not null then
    return;
  end if;

  if v_order.cashback_used > 0 then
    update public.customers set cashback_balance = cashback_balance + v_order.cashback_used
      where id = v_order.customer_id;
  end if;

  if v_order.cashback_credited_at is not null then
    if v_order.cashback_earned > 0 then
      update public.customers set cashback_balance = greatest(cashback_balance - v_order.cashback_earned, 0)
        where id = v_order.customer_id;
    end if;
    if v_order.referral_bonus_earned > 0 and v_order.referrer_customer_id is not null then
      update public.customers set cashback_balance = greatest(cashback_balance - v_order.referral_bonus_earned, 0)
        where id in (v_order.customer_id, v_order.referrer_customer_id);
    end if;
  end if;

  update public.orders set cashback_reversed_at = now() where id = p_order_id;
end;
$$;

revoke execute on function public.reverse_order_cashback(uuid) from public, anon, authenticated;
