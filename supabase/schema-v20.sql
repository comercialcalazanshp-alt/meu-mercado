-- Meu Mercado — v20.
-- "Compre 2, leve 3" automático: promoção opcional por produto (leve X,
-- pague Y). Some ao carrinho — no site e no PDV — sem o dono precisar
-- criar cupom ou kit pra isso. Preço final sempre calculado no servidor,
-- igual estoque e preço normal, nunca confia no navegador.

alter table products add column if not exists promo_buy_qty integer;
alter table products add column if not exists promo_pay_qty integer;

alter table products drop constraint if exists products_promo_check;
alter table products add constraint products_promo_check
  check (
    (promo_buy_qty is null and promo_pay_qty is null)
    or (promo_buy_qty is not null and promo_pay_qty is not null
        and promo_pay_qty > 0 and promo_buy_qty > promo_pay_qty)
  );
...
begin
  ...
  loop
    ...
    if v_item ? 'kit_id' then
      ...
      select id, name, price, stock, promo_buy_qty, promo_pay_qty into v_product
      from public.products
      ...
      if v_product.promo_buy_qty is not null and v_quantity >= v_product.promo_buy_qty then
        v_line_total := (
