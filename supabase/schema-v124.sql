-- Meu Mercado — v124.
-- Ajuste fino em checkout(): o pedido só gravava coupon_code quando o
-- desconto calculado dava mais que zero. Se um cupom válido resultasse em
-- desconto R$ 0,00 (ex: cupom mal configurado, ou percentual muito baixo
-- num carrinho pequeno), o pedido ficava sem coupon_code — e a checagem
-- de "uso único por cliente" (que procura por coupon_code) nunca
-- encontrava esse uso anterior, permitindo reusar o mesmo cupom
-- indefinidamente nesse caso específico. Agora estampa o código sempre
-- que um cupom válido foi de fato aplicado, independente do valor do
-- desconto.
...
begin
  ...
  values (
    ...
    v_order_items,
    v_total,
    case when v_coupon_applied then upper(trim(p_coupon_code)) else null end,
    v_discount,
    v_customer.id,
    v_cashback_earned,
    v_cashback_used,
