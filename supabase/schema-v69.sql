-- Meu Mercado — v69.
-- Receita da semana já tinha "curtida"/destaque, mas o dono não sabia se
-- aquilo realmente virava venda — só via de longe, sem número. recipe_clicks
-- registra cada clique em "Adicionar ingredientes ao carrinho"; orders.recipe_id
-- guarda, quando o pedido nasceu de um clique desses, qual receita foi. Dá
-- pra cruzar os dois e mostrar cliques → pedidos → taxa de conversão por
-- receita, igual já existe pra banner/oferta.
create table if not exists public.recipe_clicks (
...
create index if not exists idx_recipe_clicks_recipe on public.recipe_clicks(recipe_id);
create index if not exists idx_recipe_clicks_store on public.recipe_clicks(store_id);

alter table public.recipe_clicks enable row level security;

drop policy if exists "qualquer um registra clique de receita" on public.recipe_clicks;
create policy "qualquer um registra clique de receita" on public.recipe_clicks for insert
  with check (
    exists (select 1 from public.recipes r where r.id = recipe_clicks.recipe_id and r.store_id = recipe_clicks.store_id)
  );

drop policy if exists "dono ve cliques de receita da propria loja" on public.recipe_clicks;
create policy "dono ve cliques de receita da propria loja" on public.recipe_clicks for select
  using (store_id in (select public.my_store_ids()));

alter table public.orders add column if not exists recipe_id uuid references public.recipes(id) on delete set null;
