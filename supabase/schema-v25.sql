-- Meu Mercado — v25.
-- Ofertas do Dia: marcar produto em oferta com preço reduzido. Quando
-- ativo, o preço da oferta manda sozinho no carrinho (não soma com
-- atacado/leve-mais-pague-menos — é a promoção mais direta, pensada pra
-- durar só um dia ou uma semana).

alter table products add column if not exists on_offer boolean not null default false;
...
alter table products add constraint products_offer_check
  check (not on_offer or (offer_price is not null and offer_price >= 0 and offer_price < price));
