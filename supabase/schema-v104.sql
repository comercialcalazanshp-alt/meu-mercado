-- Meu Mercado — v104.
-- Nota interna + bloqueio de cliente. A tela de Clientes junta gente de
-- "customers" (quem já comprou pelo site) e "credit_customers" (cadastro
-- manual de fiado) pelo telefone — nem todo cliente ali tem uma linha
-- garantida em "customers". Por isso uma tabela própria, chave (store_id,
-- phone), em vez de tentar pendurar em uma tabela só.
...
-- checkout() passa a recusar pedido de cliente bloqueado (bloqueio é por
-- telefone, dentro da própria loja).
