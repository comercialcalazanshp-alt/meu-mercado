-- Meu Mercado — v53.
-- Corrige um furo sério: uma conta de DONO de loja (criada em /cadastro,
-- sem nenhuma linha em customer_profiles) conseguia "logar" na área de
-- cliente do site (/cliente/entrar) usando o mesmo e-mail/senha — porque
-- login no Supabase Auth não sabe nada sobre account_type, isso só existe
-- nos metadados usados pelo gatilho de cadastro. O JWT autentica igual pra
-- qualquer conta do mesmo projeto, dono ou cliente.
--
-- Consequência real: checkout() e get_or_create_scratch_card() confiavam
-- cegamente em "auth.uid() não é nulo" pra decidir se cashback/indicação/
-- raspadinha valiam — e ao tentar linkar profile_id de um dono (que não
-- existe em customer_profiles), a foreign key derrubava o checkout inteiro
-- com erro, quebrando pedidos de qualquer loja pra quem caísse nesse caso.
--
-- Correção: nunca mais confiar só em "tem um JWT válido" — sempre confirmar
-- que existe mesmo uma linha em customer_profiles pra esse auth.uid() antes
-- de tratar como cliente de verdade.
...
begin
  -- Um JWT válido só conta como "cliente logado" se existir mesmo uma conta
  -- de cliente registrada com esse id — fecha a brecha de uma conta de dono
  -- de loja (ou qualquer outra sem perfil de cliente) ser tratada como
  -- cliente só por ter uma sessão autenticada.
...
declare
  ...
  v_pick numeric := null;
begin
  if auth.uid() is null or not exists (select 1 from public.customer_profiles where id = auth.uid()) then
