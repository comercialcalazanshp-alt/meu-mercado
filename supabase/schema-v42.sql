-- PDV: produtos vendidos por peso (kg), com atalho de digitar
-- "500," (gramas) ou "5*" (quantidade de unidades) antes de escolher o produto.
...
-- v40/v41 added parameters to pdv_sale, leaving old-signature overloads behind
-- (create or replace only replaces an exact signature match). PostgREST can't
-- pick between overloads when a caller omits the newer optional params, so
-- drop the stale ones before recreating the current signature below.
drop function if exists public.pdv_sale(uuid, jsonb, text, text, text);
drop function if exists public.pdv_sale(uuid, jsonb, text, text, text, jsonb);
