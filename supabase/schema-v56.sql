drop function if exists public.get_or_create_scratch_card(uuid);

-- O desconto da raspadinha passa a valer só por 48h a partir do momento
-- que o cliente raspa (em vez de "até o fim da semana", que dava dias a
-- mais pra quem raspava no começo da semana e quase nada pra quem raspava
-- no fim). Continua sendo só 1 raspada por semana — só muda o prazo pra
-- usar o desconto ganho.
...
begin
  ...
  return query select v_pick, false, false, false;
end;
$$;

-- checkout() só aplica o desconto se ainda estiver dentro das 48h — mantém
-- o "1 por semana" (o card só existe se a pessoa já raspou essa semana),
-- mas expira o desconto depois de 48h mesmo que ainda seja a mesma semana.
